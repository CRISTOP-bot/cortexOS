...
- `third_party/doom` is a pinned submodule containing the upstream GPLv2 source.
- `usr/doom/cortexos_platform.h` and `.c` define a CortexOS-owned platform
...
`third_party/doom/LICENSE.TXT` before redistributing a combined work. CortexOS
...
| Compiler/libc/userspace | GCC-hosted freestanding kernel; small static ELF/libc smoke tests in `usr/`; no complete POSIX process runtime | Engine needs a bounded libc/ABI port, argument/env setup, heap and process launch |
...
| Keyboard | x86_64 PS/2 set-1 IRQ path and scancode API (`kernel/include/keyboard.h`) | The mapping in `usr/doom` is ready for an adapter, but event delivery is not |
...
| Build system | Makefile builds x86_64 kernel/ISO, userspace smoke tests, and early ARM images | Doom source build and engine object list remain future work |
