...
:install_choco
    ...
    echo.
    echo     %C%*%R%  Or use tools/setup/install-deps.sh inside MSYS2/WSL.
    goto :eof
...
:install_wsl
    ...
    echo     %C%*%R%  Running tools/setup/install-deps.sh inside WSL...
    echo.

    :: Copy script to WSL and run it
    wsl -d !WSL_DISTRO! -- bash -c "curl -sL https://raw.githubusercontent.com/CRISTOP-bot/nucleos/main/tools/setup/install-deps.sh | bash" 2>&1
    if !errorlevel! equ 0 (
