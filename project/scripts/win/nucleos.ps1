...
function Invoke-Python([string[]]$Arguments) {
    ...
    if (-not $python) { throw 'No se encontró Python. Ejecuta project\tools\setup\install-deps.bat.' }
...
function Invoke-Make([string[]]$Arguments) {
    ...
    if (-not $wsl) {
        throw 'No se encontró make.exe ni wsl.exe. Ejecuta project\tools\setup\install-deps.bat.'
    }
...
switch ($Command) {
    'check' {
        & (Join-Path $Root 'project\tools\setup\install-deps.bat') --check
        Invoke-Python @((Join-Path $Root 'project\scripts\linux\check-layout.py'))
    }
