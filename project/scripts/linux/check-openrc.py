...
def load_stager():
    spec = importlib.util.spec_from_file_location("openrc_stager", ROOT / "project/tools/build/openrc.py")
...
def main() -> int:
    config = ROOT / "project/config/openrc/cortexos-x86_64.ini"
    ...
    for token in ("c = 'x86_64-cortexos-gcc'", "pam = false", "selinux = 'disabled'", "pkgconfig = false"):
        if token not in config_text:
            ...
            return 1
    if not (ROOT / "userspace/rootfs/etc/init.d/nucleos-console").is_file():
        print("missing CortexOS service script", file=sys.stderr)
        return 1
    if not (ROOT / "userspace/rootfs/etc/conf.d/openrc").is_file():
