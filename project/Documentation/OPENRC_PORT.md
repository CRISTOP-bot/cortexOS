...
- Ruta: `project/third_party/openrc/`

La rama conserva un contrato reproducible para la siguiente etapa del port:

- `project/config/openrc/cortexos-x86_64.ini` es una plantilla Meson que identifica a
  CortexOS como sistema destino y nunca usa el compilador Linux del host.
- `project/tools/build/openrc.py` valida y copia únicamente los cuatro ejecutables
...
- `make openrc-stage OPENRC_BIN_DIR=...` instala esos binarios en el rootfs,
  ...
  host.
- `userspace/rootfs/etc/conf.d/openrc` y el árbol de configuración del runlevel default
...
El kernel conserva el gestor de compatibilidad de `boot/openrc/openrc.c` para
